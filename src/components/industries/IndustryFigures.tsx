...
export function ManufacturingArt({ compact, className, label }: ArtProps) {
  ...
  const P = projector(229.5, 160, 1.1);
  const at = (x: number, y: number, z = 0) => P(x, y, z);
  const X = (x: number, y: number, z = 0) => r1(P(x, y, z)[0]);
  const Y = (x: number, y: number, z = 0) => r1(P(x, y, z)[1]);
  ...
  // Sawtooth hall: four north-light teeth along the back wall.
  const teeth = [0, 1, 2, 3].map((i) => [64 + i * 39, 64 + (i + 1) * 39] as const);
  const railTag = (x: number) => ({ x: X(x, 116), y: Y(x, 116) });

  return (
    <Plate compact={compact} className={className} label={label} viewBox="0 0 640 480" compactViewBox="56 52 528 330">
      ...
      <Plate3D P={P} w={330} d={140} />

      {/* The production hall behind the line: north-light sawtooth roof. */}
      <Stage id="production">
        <path className={s.surface} d={qL(P, 30, 64, 220, 0, 30)} />
        <path className={s.soft} d={qR(P, 220, 4, 30, 0, 30)} />
        {teeth.map(([x0, x1]) => <g key={x0}>
          <path className={s.paper} d={poly([P(x0, 4, 30), P(x1, 4, 42), P(x1, 30, 42), P(x0, 30, 30)])} />
          <path className={s.glass} d={qR(P, x1, 4, 30, 30, 42)} />
          <path className={s.surface} d={poly([P(x0, 30, 30), P(x1, 30, 42), P(x1, 30, 30)])} />
        </g>)}
        {[76, 100, 124, 148, 172, 196].map((x) => <path key={x} className={s.glass} d={qL(P, 30, x, x + 14, 15, 22)} />)}
      </Stage>

      {/* 03 — raw-material godown; greige rolls wait by the line. */}
      <Stage id="material">
        <Halo d={footprint(P, 8, 34, 64, 50)} />
        ...
        {[[14, 24], [30, 40]].map(([x0, x1]) => <path key={x0} className={s.glass} d={qL(P, 84, x0, x1, 17, 23)} />)}
        <path className={s.soft} d={qL(P, 84, 42, 52, 0, 13)} />
        <Cyl P={P} x={62} y={38} r={5} h={13} className={s.tintMuted} />
        <Cyl P={P} x={64} y={47} r={5} h={11} />
        <Tap x={X(32, 59, 44)} y={r1(Y(32, 59, 44) - 10)} mast={7} />
      </Stage>

      {/* Conveyor along the line; goods ride it. */}
      <Stage id="production">
        <Block P={P} x={56} y={52} z={6} w={164} d={16} h={6} />
      ...
      <Stage id="production">
        <Halo d={footprint(P, 72, 40, 96, 42)} />
        <Link d={belt(58, 130)} delaySec={-1} />
        <path className={cx(s.goods, s.ambient)} d={belt(58, 220)} />
        <Block P={P} x={74} y={44} w={30} d={32} h={36} />
        <path className={s.glass} d={qL(P, 76, 78, 96, 10, 29)} />
        <path className={s.inkLine} d={polyline([P(87, 76, 29), P(87, 76, 20)])} />
        <path className={s.accentLine} d={polyline([P(87, 76, 19), P(87, 76, 16)])} />
        <path className={s.machineSlot} d={qR(P, 104, 49, 63, 19, 30)} />
        <path className={s.whiteLine} d={`${polyline([P(104, 51, 26), P(104, 58, 26)])}${polyline([P(104, 51, 23), P(104, 55, 23)])}`} />
        <path className={s.tint} d={qT(P, 36, 80, 50, 94, 62)} />
        <circle className={cx(s.accentFill, s.blink)} cx={X(104, 69, 26)} cy={Y(104, 69, 26)} r="1.8" />
        {[[108, 12], [108, 19], [118, 12]].map(([x, z], k) => <Block key={k} P={P} x={x} y={54} z={z} w={8} d={10} h={7} tone={k === 1 ? "tint" : "paper"} />)}
        <Block P={P} x={130} y={42} w={36} d={36} h={10} />
        <Block P={P} x={132} y={44} z={10} w={6} d={6} h={36} />
        <Block P={P} x={158} y={44} z={10} w={6} d={6} h={36} />
        <Block P={P} x={130} y={42} z={46} w={36} d={12} h={10} />
        <g className={s.press} style={cssVars({ "--stroke": "9px" })}>
          <Block P={P} x={139} y={52} z={32} w={18} d={16} h={8} tone="tint" />
        </g>
        <Block P={P} x={132} y={70} z={10} w={6} d={6} h={36} />
        <Block P={P} x={158} y={70} z={10} w={6} d={6} h={36} />
        <Block P={P} x={130} y={66} z={46} w={36} d={12} h={10} />
        <Tap x={X(89, 60, 36)} y={r1(Y(89, 60, 36) - 12)} mast={9} />
        <Tap x={X(148, 60, 56)} y={r1(Y(148, 60, 56) - 10)} mast={7} />
      </Stage>

      {/* 05 — quality gate: a light curtain over the belt. */}
      <Stage id="qc">
        <Halo d={footprint(P, 180, 44, 14, 32)} />
        <Link d={belt(166, 186)} delaySec={-3} fast />
        {[46, 68].map((y) => <Block key={y} P={P} x={183} y={y} w={7} d={6} h={32} />)}
        <path className={cx(s.tintFill, s.blink)} d={qR(P, 186.5, 52, 68, 13, 31)} />
        <path className={s.accentLine} strokeDasharray="1.5 3" d={[17, 22, 27].map((z) => polyline([P(186.5, 52, z), P(186.5, 68, z)])).join("")} />
        <Block P={P} x={181} y={44} z={32} w={11} d={32} h={5} />
        <circle className={s.accentFill} cx={X(187, 60, 37)} cy={r1(Y(187, 60, 37) - 2)} r="2.2" />
        <circle className={cx(s.accentLine, s.pulse)} cx={X(187, 60, 37)} cy={r1(Y(187, 60, 37) - 2)} r="2.2" />
        {[198, 207].map((x) => <Block key={x} P={P} x={x} y={54} z={12} w={7} d={10} h={7} />)}
      </Stage>

      {/* 06 — finished-goods godown and the truck at its dock. */}
      <Stage id="dispatch">
        <Halo d={footprint(P, 224, 34, 100, 50)} />
        <Link d={`${belt(190, 224)}${polyline([P(272, 60, 2), P(278, 60, 2)])}`} delaySec={-5} />
        <GableX P={P} x={224} y={34} w={48} d={50} h={28} rh={12} tone="tint" />
        <path className={s.soft} d={qR(P, 272, 50, 70, 0, 20)} />
        <path className={s.line} d={[5, 10, 15].map((z) => polyline([P(272, 50, z), P(272, 70, z)])).join("")} />
        {[[232, 242], [254, 264]].map(([x0, x1]) => <path key={x0} className={s.glass} d={qL(P, 84, x0, x1, 15, 21)} />)}
        <Block P={P} x={278} y={48} z={4} w={32} d={24} h={22} />
        <path className={s.accentLine} d={polyline([P(284, 72, 16), P(302, 72, 16)])} />
        <path className={s.line} d={polyline([P(284, 72, 11), P(296, 72, 11)])} />
        <Block P={P} x={310} y={50} z={4} w={14} d={22} h={17} />
        <path className={s.glass} d={qR(P, 324, 53, 69, 11, 18)} />
        <path className={s.glass} d={qL(P, 72, 313, 321, 11, 18)} />
        {[286, 302, 317].map((x) => <path key={x} className={s.wheel} d={ring(P, "y", [x, 72.5, 4.5], 4.5, 20)} />)}
      </Stage>

      {/* 01 — the sales order. */}
      <Stage id="order">
        <Panel x={48} y={60} w={120} h={58} n={n.order} title="SALES ORDER">
          <text className={s.micro} x="56" y="92">SO-1042</text>
          <text className={s.microInk} x="160" y="92" textAnchor="end">1,200 M</text>
          <path className={s.bar} style={{ strokeWidth: 3.4, strokeLinecap: "butt" }} d="M56 102H86" />
          <path className={s.barAccent} style={{ strokeWidth: 3.4, strokeLinecap: "butt" }} d="M88 102H160" />
          <text className={s.micro} x="56" y="113">STOCK 400</text>
          <text className={s.microAccent} x="160" y="113" textAnchor="end">MAKE 800</text>
        </Panel>
      </Stage>

      {/* 02 — production plan: the work order lands on Thursday, line 2. */}
      <Stage id="plan">
        <Link d="M108 118V128" ambient={false} />
        <Panel x={48} y={128} w={120} h={62} n={n.plan} title="PLAN" code="WO-0318">
          {["M", "T", "W", "T", "F", "S"].map((d, k) => <text key={k} className={k === 3 ? s.microAccent : s.micro} x={r1(62 + k * 18.2)} y="158" textAnchor="middle">{d}</text>)}
          <path className={cx(s.guide, s.dashed)} d="M56 165H160M56 174H160M56 183H160" />
          <path className={s.barSoft} style={{ strokeWidth: 5, strokeLinecap: "butt" }} d="M56 165H90M98 174H120M56 183H78M128 183H156" />
          <path className={s.bar} style={{ strokeWidth: 5, strokeLinecap: "butt" }} d="M94 165H118" />
          <path className={s.barAccent} style={{ strokeWidth: 5, strokeLinecap: "butt" }} d="M110 174H148" />
        </Panel>
        <Link d={`M168 150H${X(32, 59, 44)}V${r1(Y(32, 59, 44) - 13)}`} delaySec={-2} fast />
      </Stage>

      {/* 07 — e-invoice + e-way bill, generated from the dispatch data. */}
      <Stage id="invoice">
        <Link d={`M${X(302, 60, 26)} ${r1(Y(302, 60, 26) - 2)}V232`} delaySec={-4} fast />
      ...
      <Stage id="dashboard">
        ...
        <Panel x={452} y={60} w={140} h={96} n={n.dashboard} title={"P&L · TODAY"} code="LIVE">
          <text className={s.figureNum} x="460" y="100">18.4%</text>
      ...
      {/* Floor tags, on a rail in front of the line. */}
      <Stage id="material"><Tag n={n.material} name="MATERIAL" {...railTag(14)} leader={polyline([at(14, 86), at(14, 110)])} /></Stage>
      <Stage id="production"><Tag n={n.production} name="PRODUCTION" {...railTag(118)} anchor="end" leader={polyline([at(118, 80), at(118, 110)])} /></Stage>
      <Stage id="qc"><Tag n={n.qc} name="QC" {...railTag(187)} anchor="end" leader={polyline([at(187, 76), at(187, 110)])} /></Stage>
      <Stage id="dispatch"><Tag n={n.dispatch} name="DISPATCH" {...railTag(262)} anchor="end" leader={polyline([at(262, 86), at(262, 110)])} /></Stage>
    </Plate>
  );
}
