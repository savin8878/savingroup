...
/** Motion only animates diagram strokes; every number and quote is in the server HTML. */
export function CaseStudiesMotion({ children, labels }: { children: ReactNode; labels: { pause: string; resume: string; reduced: string } }) {
  ...
  useEffect(() => {
    ...
    root.current?.querySelectorAll("[data-case-scene]").forEach((node) => observer.observe(node));
  ...
  return <div className={`${home.root} ${styles.root}`} ref={root} data-case-studies-page data-motion={paused || reduced || background ? "paused" : "running"}>
    {children}
