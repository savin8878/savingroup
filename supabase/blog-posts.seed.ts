/**
 * Original hard-coded blog posts. Used only to generate supabase/seed.sql
 * (run: node supabase/generate-seed.ts). The site reads posts from Supabase.
 */
import type { BlogPost } from "../src/lib/blogs.ts";
...
export const BLOG_POSTS: BlogPost[] = [
  ...
  // ------------------------------------------------------------------------
  // Procurement automation — the home page's PR-0241 walkthrough, in long form
  // ------------------------------------------------------------------------
  {
    slug: "procurement-automation-requisition-to-receipt",
    title: "Procurement Automation for SMEs: Follow One Order from Requisition to Goods Receipt",
    subtitle:
      "Requisition, approval, RFQ, purchase order, dispatch, goods receipt — the stages where purchasing leaks money in a WhatsApp-and-Tally business, and how one connected record trail closes each gap.",
    excerpt:
      "Most SME purchasing runs on WhatsApp voice notes, quotes saved as photos and a Tally entry made after the goods arrive. Follow one sample order — PR-0241 for 24 drive assemblies — through eight connected stages, and see where automation earns its keep and where people should stay in charge.",
    category: "ops",
    readTime: 11,
    ...
    translations: {
      hi: {
        title: "SMEs के लिए Procurement Automation: Requisition से Goods Receipt तक एक Order का सफ़र",
        subtitle:
          "Requisition, approval, RFQ, purchase order, dispatch, goods receipt — WhatsApp और Tally पर चलने वाले business में वो stages जहाँ purchasing में पैसा leak होता है, और एक connected record trail हर gap को कैसे बंद करता है।",
        excerpt:
          "ज़्यादातर SME purchasing WhatsApp voice notes, photo में आए quotes और माल आने के बाद की गई Tally entry पर चलती है। एक sample order — 24 drive assemblies के लिए PR-0241 — को आठ connected stages से गुज़रते देखें, और समझें कि automation कहाँ काम आता है और फ़ैसला कहाँ इंसान के हाथ में रहना चाहिए।",
      },
      es: {
        title: "Automatización de compras para pymes: un pedido, de la solicitud a la recepción de mercancía",
        subtitle:
          "Solicitud, aprobación, RFQ, orden de compra, despacho, recepción de mercancía: las etapas donde las compras pierden dinero en un negocio que funciona con WhatsApp y Tally, y cómo un único registro conectado cierra cada hueco.",
        excerpt:
          "En la mayoría de las pymes, las compras funcionan con notas de voz de WhatsApp, cotizaciones guardadas como fotos y un asiento en Tally hecho después de que llega la mercancía. Sigue un pedido de ejemplo —PR-0241 por 24 conjuntos de accionamiento— a través de ocho etapas conectadas y descubre dónde rinde la automatización y dónde deben seguir decidiendo las personas.",
      },
      fr: {
        title: "Automatisation des achats pour les PME : une commande, de la demande d'achat à la réception",
        subtitle:
          "Demande d'achat, approbation, appel d'offres, bon de commande, expédition, réception : les étapes où les achats perdent de l'argent dans une entreprise qui tourne sur WhatsApp et Tally, et comment une trace unique et connectée referme chaque brèche.",
        excerpt:
          "Dans la plupart des PME, les achats reposent sur des notes vocales WhatsApp, des devis reçus en photo et une saisie Tally faite après l'arrivée de la marchandise. Suivez une commande d'exemple — PR-0241 pour 24 ensembles d'entraînement — à travers huit étapes connectées, et voyez où l'automatisation rapporte et où les décisions doivent rester humaines.",
      },
      de: {
        title: "Beschaffungsautomatisierung für KMU: eine Bestellung, von der Bedarfsanforderung bis zum Wareneingang",
        subtitle:
          "Bedarfsanforderung, Freigabe, Angebotsanfrage, Bestellung, Versand, Wareneingang — die Stufen, an denen der Einkauf in einem WhatsApp-und-Tally-Betrieb Geld verliert, und wie eine durchgängig verknüpfte Belegkette jede Lücke schließt.",
        excerpt:
          "In den meisten KMU läuft der Einkauf über WhatsApp-Sprachnachrichten, Angebote als Fotos und eine Tally-Buchung, die erst nach dem Wareneingang entsteht. Verfolgen Sie eine Beispielbestellung — PR-0241 über 24 Antriebsbaugruppen — durch acht verknüpfte Stufen und sehen Sie, wo sich Automatisierung lohnt und wo Menschen entscheiden sollten.",
      },
      ar: {
        title: "أتمتة المشتريات للشركات الصغيرة والمتوسطة: طلب واحد من طلب الشراء إلى استلام البضائع",
        subtitle:
          "طلب الشراء، والموافقة، وطلب عروض الأسعار، وأمر الشراء، والشحن، واستلام البضائع — المراحل التي تتسرّب فيها الأموال في شركة تعمل عبر WhatsApp وTally، وكيف يسدّ سجلّ واحد متصل كل ثغرة.",
        excerpt:
          "تعتمد المشتريات في معظم الشركات الصغيرة والمتوسطة على رسائل WhatsApp الصوتية، وعروض أسعار محفوظة كصور، وقيد في Tally يُسجَّل بعد وصول البضاعة. تابع طلبًا تجريبيًا — PR-0241 لـ 24 مجموعة محرّك — عبر ثماني مراحل متصلة، واكتشف أين تُفيد الأتمتة وأين يجب أن يبقى القرار بيد الإنسان.",
      },
      zh: {
        title: "中小企业采购自动化：跟随一张订单，从请购到收货",
        subtitle:
          "请购、审批、询价、采购订单、发运、收货——在依赖 WhatsApp 和 Tally 运转的企业里，采购在这些环节流失资金；一条互联的单据链可以逐一堵住这些缺口。",
        excerpt:
          "大多数中小企业的采购依靠 WhatsApp 语音消息、以照片形式保存的报价，以及货到之后才录入的 Tally 凭证。跟随一张示例订单——PR-0241，采购 24 套驱动总成——走完八个互联环节，看看自动化在哪里真正见效，哪些决定仍应由人来做。",
      },
    },
    author: {
      name: "Kanha Singh",
    ...
    heroSketch: "procurementFlow",
    tags: ["procurement", "erp", "automation", "ops", "manufacturing"],
    keywords: {
      primary: "procurement automation",
      secondary: [
    ...
    relatedSlugs: ["whatsapp-as-crm", "5-layer-revenue-stack", "revenue-audit-45-minutes"],
    crossPageLinks: [
      { href: "/industries/manufacturing", label: "Manufacturing ERP", note: "purchasing, inventory, GST and e-way bills in one system" },
      { href: "/services", label: "ERP & automation", note: "how we scope and build it" },
      { href: "/contact", label: "Map your purchase flow", note: "free 45-minute review" },
    ],
    faq: [
      {
        question: "What is procurement automation?",
        answer:
          "Procurement automation connects the steps of buying — requisition, approval, request for quotation, quotation comparison, purchase order, dispatch, goods receipt and invoice matching — into one record trail, so each step creates the next instead of being retyped from a chat. Routine work such as routing, reminders and matching is automated; approving spend, choosing suppliers and resolving exceptions stay with people.",
      },
      {
        question: "What is a 3-way match in procurement?",
        answer:
          "It means checking three documents before paying a supplier: the purchase order (what was agreed, at what rate), the goods receipt note (what arrived and was accepted) and the supplier's invoice (what is being billed). Quantity, rate and tax should agree. Anything that doesn't goes to the buyer as an exception. It catches short receipts, rate drift and GST mismatches before money leaves the account.",
      },
      {
        question: "Can we automate procurement if our suppliers only use WhatsApp?",
        answer:
          "Yes — design for it rather than against it. Send RFQs, purchase orders and dispatch reminders as approved WhatsApp Business templates, capture each reply against the right RFQ or PO, and route anything the system can't read to the buyer. Suppliers keep using WhatsApp; your purchase records stop living in chat history.",
      },
      {
        question: "Do we have to replace Tally to automate purchasing?",
        answer:
          "No. TallyPrime and Zoho Inventory both support purchase orders and goods receipts, and switching those features on is a good first step. When you need requisitions driven by production planning, approval rules by amount, stock across several godowns or supplier integrations, a connected ERP can run purchasing and inventory while Tally stays as the accounting book, with purchase vouchers synced instead of retyped.",
      },
      {
        question: "How long does it take to set up procurement automation for an SME?",
        answer:
          "A focused rollout takes about 30 days when it runs alongside the current process: week one to map the flow and clean the item master, week two for requisitions, approvals and numbered POs, week three for RFQs and supplier messaging, and week four for goods receipts and the 3-way match. Multi-plant setups take longer, mostly because of data clean-up.",
      },
    ],
    takeaways: [
      "Procurement automation is one connected record trail — requisition to receipt — not a bot that buys.",
      "Automate the handoffs: routing, reminders, numbering, matching. Keep people on approvals, supplier choice and exceptions.",
      "Compare the full offer against the requirement. The lowest rate that meets the delivery window beats the lowest rate.",
      "The 3-way match (PO, GRN, invoice) is where short receipts, rate drift and GST credit problems get caught before payment.",
      "Suppliers can stay on WhatsApp. It just can't be where your purchase records live.",
      "Start with the software you already have. Move to a connected ERP when locations, volume or approval rules outgrow it.",
    ],
    sections: [
      {
        heading: "Where purchasing leaks money in a WhatsApp-and-Tally business",
        paragraphs: [
          "Ask the production head of a mid-sized Indian manufacturer how purchasing works and you'll usually hear some version of this. A supervisor sends a WhatsApp voice note to the purchase team. The buyer calls two or three suppliers he trusts. Quotations arrive as photos of letterheads. Somebody types a PO in Tally or Word — sometimes after the order has already been placed by phone. And the first time accounts sees the transaction is when a supplier's invoice turns up with a delivery challan stapled to it.",
          "None of this is careless. It's how purchasing grows when the business grows faster than its systems. But every handoff in that chain happens somewhere different — a chat, a phone call, a register at the gate, a folder in accounts — and money leaks in the gaps between them. It rarely leaks in one dramatic loss. It leaks in small, repeatable ways nobody is positioned to see.",
        ],
        bullets: [
          "Duplicate or panic orders — two people order the same item because neither can see the other's request",
          "Rate drift — the invoice rate differs from the rate agreed on the call, and nobody has the quote to check against",
          "Short or damaged receipts paid in full — stores accepted 22, the invoice says 24, accounts pays for 24",
          "Late material discovered late — nobody knew the supplier hadn't dispatched until the line was waiting for it",
          "GST credit at risk — invoices that don't reconcile with GSTR-2B, or an HSN code that doesn't match the item",
        ],
        callout: {
          title: "The five-invoice test",
          body:
            "Pick last month's five largest purchase invoices. For each one, try to produce the requisition, the approval, the competing quotes, the PO and the goods receipt in under five minutes. If you can't, you don't have a procurement process yet — you have habits that mostly work.",
        },
      },
      {
        heading: "What procurement automation actually means (and what it doesn't)",
        paragraphs: [
          "Procurement automation doesn't mean a bot that buys things. For an SME it means something plainer: every purchase moves through one connected record trail — requisition, approval, request for quotation, quotation comparison, purchase order, dispatch, goods receipt, invoice — and each step creates the next one instead of being retyped from a chat.",
          "The automation lives in the handoffs. When stock falls short of what production needs, a requisition drafts itself. When the requisition is approved, the buyer gets an RFQ ready to send. When the supplier dispatches, receiving knows what's coming and against which PO. When the invoice arrives, the system already knows what was ordered and what was accepted. People stop being the glue between steps and go back to the parts of the job that need judgement.",
          "We usually write about the revenue side of an SME — [the 5-layer revenue stack](/blogs/5-layer-revenue-stack) is the map for that. Procurement is the other half of the margin. A rupee lost to a short receipt or a drifted rate never shows up as a marketing problem, but it comes off the same bottom line.",
        ],
        bullets: [
          "Automate: requisitions from stock and production rules, approval routing by amount, RFQ sending, supplier reminders, PO numbering, inventory updates from the goods receipt, and the 3-way match",
          "Keep human: approving spend, choosing a supplier when the cheapest isn't right, accepting or rejecting goods at the gate, and resolving invoice exceptions",
        ],
        pullQuote:
          "Automate the handoffs, not the judgement. A system should never commit money a person hasn't approved.",
      },
      {
        heading: "Follow one order: PR-0241, from requisition to goods receipt",
        paragraphs: [
          "The easiest way to see it is to follow a single purchase. The example below is illustrative — sample data, not a client's order — but the numbers are internally consistent, so you can follow the arithmetic from end to end.",
          "Production needs 30 drive assemblies for an upcoming run. Stores has six. The ERP raises requisition PR-0241 for the remaining 24, linked to the production requirement that created it. The department lead checks the need against budget and approves it, so the decision is recorded rather than remembered. Purchasing then sends RFQ-0186 to three qualified suppliers with the same specification, the same quantity and a seven-day delivery requirement.",
          "Three quotations come back into the same record. Supplier A quotes ₹6,500 a unit with a six-day lead time, Supplier B ₹6,750 with nine days, and Supplier C ₹6,900 with four. All three meet the specification. B drops out — not on price, but because nine days misses the seven-day window. C is fastest, but those two extra days of buffer would cost ₹9,600 on this order. A is the lowest rate that meets the requirement, so A gets PO-0089: 24 units at ₹6,500, a subtotal of ₹1,56,000 before GST.",
          "From there the order becomes a shipment plan — two pallets, a booked vehicle, a planned route — and dispatch and arrival milestones update the same PO, so the receiving team isn't waiting on a phone call. At the gate, receiving checks all 24 units against PO-0089, accepts them and posts GRN-0058. Stock moves from six to 30, and the trail reads PR-0241 → PO-0089 → GRN-0058: one purchase, every handoff connected. The walkthrough below runs the whole sequence.",
        ],
        bullets: [
          "01 Requisition — PR-0241 raised for 24 units, production to purchasing",
          "02 Approval — budget checked, department lead releases it to the buyer",
          "03 RFQ — RFQ-0186 sent to three qualified suppliers",
          "04 Quotations — rate, lead time and specification compared side by side",
          "05 Purchase order — PO-0089 issued to Supplier A and acknowledged",
          "06 Dispatch plan — SHP-0089, two pallets allocated, transport booked",
          "07 In transit — dispatch and arrival milestones update the PO",
          "08 Goods receipt — GRN-0058 posted, stock updated to 30 units",
        ],
        callout: {
          title: "Try the walkthrough",
          body:
            "Press play to watch all eight stages run in sequence, or step through them yourself at your own pace. At the dispatch and in-transit stages, switch between road, sea and air to see how the vehicle, booking and shipping documents change while the purchase order stays the same. Everything in it is sample data.",
        },
        embed: "procurement",
      },
      {
        heading: "The Indian layer: GST, e-way bills and the 3-way match",
        paragraphs: [
          "Most procurement software writing assumes a Western back office. An Indian SME has three extra realities, and a purchase system that ignores them just moves the chaos from the purchase team to accounts.",
          "The first is the 3-way match. Before an invoice is paid, three documents should agree: the purchase order (what you agreed to buy, at what rate), the goods receipt note (what actually arrived and was accepted) and the supplier's invoice (what you're being asked to pay). In the sample order that's PO-0089 for 24 units at ₹6,500, GRN-0058 with 24 of 24 accepted, and an invoice that should say the same. If receiving had rejected two damaged units, the GRN would show 22 accepted, the match would flag an invoice for 24, and accounts would raise a debit note instead of paying for goods that went back on the truck.",
          "The second is GST. If the item attracts 18% GST, PO-0089's ₹1,56,000 becomes an invoice of ₹1,84,080 — and that ₹28,080 of tax is money you want back as input tax credit. Credit depends on having actually received the goods and on the supplier's invoice showing up in your GSTR-2B. A system that ties every invoice to a GRN, and keeps the HSN code and GST rate on the item master rather than in the buyer's head, catches mismatches before the return is filed instead of after.",
          "The third is movement and payment compliance. A consignment worth more than ₹50,000 generally needs an e-way bill before it moves (some states set a higher threshold for movement within the state), so the dispatch stage should record the e-way bill number and vehicle number against the PO — not in a WhatsApp photo. And since FY 2023–24, income-tax law has tied your deduction for purchases from registered micro and small enterprises to paying them within the MSMED Act limit: 45 days with a written agreement, 15 days without. Your CA will know how it applies to your suppliers. The system's job is to put the due date on the invoice record, where it gets seen.",
          "In practice, the match itself comes down to four checks:",
        ],
        bullets: [
          "Quantity — invoiced quantity is no more than the quantity accepted on the GRN",
          "Rate — invoice rate equals the PO rate, with a small rounding tolerance rather than a percentage loophole",
          "Tax — GST rate and HSN code on the invoice match the item master",
          "Exceptions — anything that fails goes to the buyer with all three documents side by side, not to a spreadsheet",
        ],
        callout: {
          title: "The rule that pays for itself",
          body:
            "No PO, no payment. Once every supplier knows that an invoice without a PO number waits, the paperwork starts arriving correctly on its own.",
        },
      },
      {
        heading: "Your suppliers will stay on WhatsApp — plan for it",
        paragraphs: [
          "Here's what most procurement software gets wrong in India: your suppliers aren't going to log into a vendor portal. The owner of a small machining unit runs the business from his phone. If your process depends on him filling in a web form, it will drift back to phone calls within a month.",
          "So don't fight the channel — structure it. It's the same principle as [running your CRM inside WhatsApp](/blogs/whatsapp-as-crm): people keep talking where they already talk, and the system captures the structure in the background. Business-initiated messages go out through the WhatsApp Business API as pre-approved templates, replies are captured against the right RFQ or PO, and anything the system can't read goes to the buyer.",
          "One rule carries over from our [WhatsApp sales agent playbook](/blogs/whatsapp-sales-agent-in-7-days): never let automation promise something a person has to deliver. The bot can chase a dispatch date; it should never agree a rate or accept a revised delivery date on your behalf. Within that rule, four touchpoints cover most of the supplier conversation:",
        ],
        bullets: [
          "RFQ out — one template per supplier with specification, quantity and required date; the quote reply lands in the comparison, not in a chat gallery",
          "PO acknowledgement — the PO goes out as a PDF, a one-word reply marks it acknowledged, and no reply in 24 hours nudges the buyer",
          "Dispatch confirmation — a reminder the day before the promised date asks for the vehicle number, e-way bill number and a copy of the invoice",
          "Arrival — the gate enters the PO number and stores gets a GRN task with the expected quantity already filled in",
        ],
        pullQuote:
          "Suppliers can keep using WhatsApp. It just can't be where your purchase records live.",
      },
      {
        heading: "Tally, Zoho or a connected ERP: what to use when",
        paragraphs: [
          "You don't necessarily need new software to start. TallyPrime can raise purchase orders, record receipt notes and track rejections, and Zoho Inventory has purchase orders, purchase receives and bills. Many SMEs that use these tools only ever post the final purchase voucher — the PO and the receipt never make it in. Switching those features on and insisting on them gets you most of the paper trail.",
          "Where they run out is the connections: requisitions that come from production planning, approvals that change by amount or department, stock spread across several godowns, supplier messages feeding the record, and a live view of which POs are late. That's the point where a connected ERP earns its cost — and your accountant can often keep Tally for the books, with purchase vouchers synced rather than retyped.",
          "Our [manufacturing ERP work](/industries/manufacturing) covers the connected version — purchasing, inventory across godowns, GST e-invoicing and e-way bills in one system — and [our services page](/services) shows how we scope it. As a rough guide:",
        ],
        bullets: [
          "One location, a handful of regular suppliers, a few dozen POs a month — turn on the purchase features you already pay for and enforce no PO, no payment",
          "Several plants or godowns, requisitions driven by production, approval rules by value — a connected ERP, with Tally kept for accounts if your CA prefers it",
          "Supplier WhatsApp flows, custom approval rules or dashboards your ERP can't do — a thin custom layer on top, integrated rather than replaced",
        ],
      },
      {
        heading: "A 30-day rollout that doesn't stop purchasing",
        paragraphs: [
          "Purchasing can't pause while you fix it, so the rollout runs alongside the current process and replaces it one stage at a time. The rule is the one we use in [the 45-minute revenue audit](/blogs/revenue-audit-45-minutes): every change gets an owner and a date.",
          "Once the four weeks below are done, measure four things every week: requisition-to-PO cycle time, deliveries received by the PO date, invoices that match first time, and invoices that arrived without a PO. When that last number reaches zero, the process is real. If you'd like a second pair of eyes on yours, [book a 45-minute review](/contact) — we'll map your purchase flow from requisition to receipt and mark where it leaks.",
        ],
        bullets: [
          "Week 1 — map the current flow for your top 20 purchased items, clean the item master (units, HSN codes, GST rates) and write the approval matrix by amount",
          "Week 2 — go live with requisitions, approvals and numbered POs, and tell suppliers that invoices need a PO number",
          "Week 3 — move RFQs and quotation comparison into the system and switch on the supplier WhatsApp templates",
          "Week 4 — post GRNs at the gate, turn on the 3-way match and review the first batch of exceptions together",
        ],
      },
    ],
  },
